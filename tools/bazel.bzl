...
# Shim for TF-vendored rules_cuda API.
def requires_cuda_enabled(tags = []):
    # When CUDA is enabled: keep 'tags' as-is.
    # When disabled: mark target manual so it doesn't fail analysis on CPU builds.
    return if_cuda(tags, tags + ["manual"])
