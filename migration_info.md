# Migration info
Command for local testing:
```
bazel build --enable_bzlmod --noenable_workspace //:pytorch_py
```
## Direct dependencies:
* pip_deps_pyyaml
* pip_deps_typing_extensions
* pip_deps_sympy
* pip_deps_setuptools
* pip_deps_numpy
* pip_deps_requests
* rules_python
## Migration of `pip_deps_pyyaml`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pip_deps_pyyaml instantiated at:
  /home/brendan/pytorch/WORKSPACE:269:13: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pip_deps/requirements.bzl:47:20: in install_deps
Repository rule whl_library defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip_install/pip_repository.bzl:712:30: in <toplevel>

```

#### Definition
```python
load("@@rules_python//python/pip_install:pip_repository.bzl", "whl_library")
whl_library(
  name = "pip_deps_pyyaml",
  repo = "pip_deps",
  requirement = "pyyaml==6.0.1     --hash=sha256:04ac92ad1925b2cff1db0cfebffb6ffc43457495c9b3c39d3fcae417d7125dc5     --hash=sha256:062582fca9fabdd2c8b54a3ef1c978d786e0f6b3a1510e0ac93ef59e0ddae2bc     --hash=sha256:0d3304d8c0adc42be59c5f8a4d9e3d7379e6955ad754aa9d6ab7a398b59dd1df     --hash=sha256:1635fd110e8d85d55237ab316b5b011de701ea0f29d07611174a1b42f1444741     --hash=sha256:184c5108a2aca3c5b3d3bf9395d50893a7ab82a38004c8f61c258d4428e80206     --hash=sha256:18aeb1bf9a78867dc38b259769503436b7c72f7a1f1f4c93ff9a17de54319b27     --hash=sha256:1d4c7e777c441b20e32f52bd377e0c409713e8bb1386e1099c2415f26e479595     --hash=sha256:1e2722cc9fbb45d9b87631ac70924c11d3a401b2d7f410cc0e3bbf249f2dca62     --hash=sha256:1fe35611261b29bd1de0070f0b2f47cb6ff71fa6595c077e42bd0c419fa27b98     --hash=sha256:28c119d996beec18c05208a8bd78cbe4007878c6dd15091efb73a30e90539696     --hash=sha256:326c013efe8048858a6d312ddd31d56e468118ad4cdeda36c719bf5bb6192290     --hash=sha256:40df9b996c2b73138957fe23a16a4f0ba614f4c0efce1e9406a184b6d07fa3a9     --hash=sha256:42f8152b8dbc4fe7d96729ec2b99c7097d656dc1213a3229ca5383f973a5ed6d     --hash=sha256:49a183be227561de579b4a36efbb21b3eab9651dd81b1858589f796549873dd6     --hash=sha256:4fb147e7a67ef577a588a0e2c17b6db51dda102c71de36f8549b6816a96e1867     --hash=sha256:50550eb667afee136e9a77d6dc71ae76a44df8b3e51e41b77f6de2932bfe0f47     --hash=sha256:510c9deebc5c0225e8c96813043e62b680ba2f9c50a08d3724c7f28a747d1486     --hash=sha256:5773183b6446b2c99bb77e77595dd486303b4faab2b086e7b17bc6bef28865f6     --hash=sha256:596106435fa6ad000c2991a98fa58eeb8656ef2325d7e158344fb33864ed87e3     --hash=sha256:6965a7bc3cf88e5a1c3bd2e0b5c22f8d677dc88a455344035f03399034eb3007     --hash=sha256:69b023b2b4daa7548bcfbd4aa3da05b3a74b772db9e23b982788168117739938     --hash=sha256:6c22bec3fbe2524cde73d7ada88f6566758a8f7227bfbf93a408a9d86bcc12a0     --hash=sha256:704219a11b772aea0d8ecd7058d0082713c3562b4e271b849ad7dc4a5c90c13c     --hash=sha256:7e07cbde391ba96ab58e532ff4803f79c4129397514e1413a7dc761ccd755735     --hash=sha256:81e0b275a9ecc9c0c0c07b4b90ba548307583c125f54d5b6946cfee6360c733d     --hash=sha256:855fb52b0dc35af121542a76b9a84f8d1cd886ea97c84703eaa6d88e37a2ad28     --hash=sha256:8d4e9c88387b0f5c7d5f281e55304de64cf7f9c0021a3525bd3b1c542da3b0e4     --hash=sha256:9046c58c4395dff28dd494285c82ba00b546adfc7ef001486fbf0324bc174fba     --hash=sha256:9eb6caa9a297fc2c2fb8862bc5370d0303ddba53ba97e71f08023b6cd73d16a8     --hash=sha256:a08c6f0fe150303c1c6b71ebcd7213c2858041a7e01975da3a99aed1e7a378ef     --hash=sha256:a0cd17c15d3bb3fa06978b4e8958dcdc6e0174ccea823003a106c7d4d7899ac5     --hash=sha256:afd7e57eddb1a54f0f1a974bc4391af8bcce0b444685d936840f125cf046d5bd     --hash=sha256:b1275ad35a5d18c62a7220633c913e1b42d44b46ee12554e5fd39c70a243d6a3     --hash=sha256:b786eecbdf8499b9ca1d697215862083bd6d2a99965554781d0d8d1ad31e13a0     --hash=sha256:ba336e390cd8e4d1739f42dfe9bb83a3cc2e80f567d8805e11b46f4a943f5515     --hash=sha256:baa90d3f661d43131ca170712d903e6295d1f7a0f595074f151c0aed377c9b9c     --hash=sha256:bc1bf2925a1ecd43da378f4db9e4f799775d6367bdb94671027b73b393a7c42c     --hash=sha256:bd4af7373a854424dabd882decdc5579653d7868b8fb26dc7d0e99f823aa5924     --hash=sha256:bf07ee2fef7014951eeb99f56f39c9bb4af143d8aa3c21b1677805985307da34     --hash=sha256:bfdf460b1736c775f2ba9f6a92bca30bc2095067b8a9d77876d1fad6cc3b4a43     --hash=sha256:c8098ddcc2a85b61647b2590f825f3db38891662cfc2fc776415143f599bb859     --hash=sha256:d2b04aac4d386b172d5b9692e2d2da8de7bfb6c387fa4f801fbf6fb2e6ba4673     --hash=sha256:d483d2cdf104e7c9fa60c544d92981f12ad66a457afae824d146093b8c294c54     --hash=sha256:d858aa552c999bc8a8d57426ed01e40bef403cd8ccdd0fc5f6f04a00414cac2a     --hash=sha256:e7d73685e87afe9f3b36c799222440d6cf362062f78be1013661b00c5c6f678b     --hash=sha256:f003ed9ad21d6a4713f0a9b5a7a0a79e08dd0f221aff4525a2be4c346ee60aab     --hash=sha256:f22ac1c3cac4dbc50079e965eba2c1058622631e526bd9afd45fedd49ba781fa     --hash=sha256:faca3bdcf85b2fc05d06ff3fbc1f83e1391b3e724afa3feba7d13eeab355484c     --hash=sha256:fca0e3a251908a499833aa292323f32437106001d436eca0e6e7833256674585     --hash=sha256:fd1592b3fdf65fff2ad0004b5e363300ef59ced41c2e6b3a99d4089fa8c5435d     --hash=sha256:fd66fc5d0da6d9815ba2cebeb4205f95818ff4b79c3ebe268e75d961704af52f",
  download_only = False,
  enable_implicit_namespace_pkgs = False,
  environment = {  },
  extra_pip_args = [  ],
  isolated = True,
  pip_data_exclude = [  ],
  python_interpreter = "python3",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
  quiet = True,
  repo_prefix = "pip_deps_",
  timeout = 600,
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `pip_deps_typing_extensions`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pip_deps_typing_extensions instantiated at:
  /home/brendan/pytorch/WORKSPACE:269:13: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pip_deps/requirements.bzl:47:20: in install_deps
Repository rule whl_library defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip_install/pip_repository.bzl:712:30: in <toplevel>

```

#### Definition
```python
load("@@rules_python//python/pip_install:pip_repository.bzl", "whl_library")
whl_library(
  name = "pip_deps_typing_extensions",
  repo = "pip_deps",
  requirement = "typing-extensions==4.11.0     --hash=sha256:83f085bd5ca59c80295fc2a82ab5dac679cbe02b9f33f7d83af68e241bea51b0     --hash=sha256:c1f94d72897edaf4ce775bb7558d5b79d8126906a14ea5ed1635921406c0387a",
  download_only = False,
  enable_implicit_namespace_pkgs = False,
  environment = {  },
  extra_pip_args = [  ],
  isolated = True,
  pip_data_exclude = [  ],
  python_interpreter = "python3",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
  quiet = True,
  repo_prefix = "pip_deps_",
  timeout = 600,
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `pip_deps_sympy`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pip_deps_sympy instantiated at:
  /home/brendan/pytorch/WORKSPACE:269:13: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pip_deps/requirements.bzl:47:20: in install_deps
Repository rule whl_library defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip_install/pip_repository.bzl:712:30: in <toplevel>

```

#### Definition
```python
load("@@rules_python//python/pip_install:pip_repository.bzl", "whl_library")
whl_library(
  name = "pip_deps_sympy",
  repo = "pip_deps",
  requirement = "sympy==1.12     --hash=sha256:c3588cd4295d0c0f603d0f2ae780587e64e2efeedb3521e46b9bb1d08d184fa5     --hash=sha256:ebf595c8dac3e0fdc4152c51878b498396ec7f30e7a914d6071e674d49420fb8",
  download_only = False,
  enable_implicit_namespace_pkgs = False,
  environment = {  },
  extra_pip_args = [  ],
  isolated = True,
  pip_data_exclude = [  ],
  python_interpreter = "python3",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
  quiet = True,
  repo_prefix = "pip_deps_",
  timeout = 600,
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `pip_deps_setuptools`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pip_deps_setuptools instantiated at:
  /home/brendan/pytorch/WORKSPACE:269:13: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pip_deps/requirements.bzl:47:20: in install_deps
Repository rule whl_library defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip_install/pip_repository.bzl:712:30: in <toplevel>

```

#### Definition
```python
load("@@rules_python//python/pip_install:pip_repository.bzl", "whl_library")
whl_library(
  name = "pip_deps_setuptools",
  repo = "pip_deps",
  requirement = "setuptools==70.3.0     --hash=sha256:f171bab1dfbc86b132997f26a119f6056a57950d058587841a0082e8830f9dc5     --hash=sha256:fe384da74336c398e0d956d1cae0669bc02eed936cdb1d49b57de1990dc11ffc",
  download_only = False,
  enable_implicit_namespace_pkgs = False,
  environment = {  },
  extra_pip_args = [  ],
  isolated = True,
  pip_data_exclude = [  ],
  python_interpreter = "python3",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
  quiet = True,
  repo_prefix = "pip_deps_",
  timeout = 600,
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `pip_deps_numpy`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pip_deps_numpy instantiated at:
  /home/brendan/pytorch/WORKSPACE:269:13: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pip_deps/requirements.bzl:47:20: in install_deps
Repository rule whl_library defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip_install/pip_repository.bzl:712:30: in <toplevel>

```

#### Definition
```python
load("@@rules_python//python/pip_install:pip_repository.bzl", "whl_library")
whl_library(
  name = "pip_deps_numpy",
  repo = "pip_deps",
  requirement = "numpy==1.26.4     --hash=sha256:03a8c78d01d9781b28a6989f6fa1bb2c4f2d51201cf99d3dd875df6fbd96b23b     --hash=sha256:08beddf13648eb95f8d867350f6a018a4be2e5ad54c8d8caed89ebca558b2818     --hash=sha256:1af303d6b2210eb850fcf03064d364652b7120803a0b872f5211f5234b399f20     --hash=sha256:1dda2e7b4ec9dd512f84935c5f126c8bd8b9f2fc001e9f54af255e8c5f16b0e0     --hash=sha256:2a02aba9ed12e4ac4eb3ea9421c420301a0c6460d9830d74a9df87efa4912010     --hash=sha256:2e4ee3380d6de9c9ec04745830fd9e2eccb3e6cf790d39d7b98ffd19b0dd754a     --hash=sha256:3373d5d70a5fe74a2c1bb6d2cfd9609ecf686d47a2d7b1d37a8f3b6bf6003aea     --hash=sha256:47711010ad8555514b434df65f7d7b076bb8261df1ca9bb78f53d3b2db02e95c     --hash=sha256:4c66707fabe114439db9068ee468c26bbdf909cac0fb58686a42a24de1760c71     --hash=sha256:50193e430acfc1346175fcbdaa28ffec49947a06918b7b92130744e81e640110     --hash=sha256:52b8b60467cd7dd1e9ed082188b4e6bb35aa5cdd01777621a1658910745b90be     --hash=sha256:60dedbb91afcbfdc9bc0b1f3f402804070deed7392c23eb7a7f07fa857868e8a     --hash=sha256:62b8e4b1e28009ef2846b4c7852046736bab361f7aeadeb6a5b89ebec3c7055a     --hash=sha256:666dbfb6ec68962c033a450943ded891bed2d54e6755e35e5835d63f4f6931d5     --hash=sha256:675d61ffbfa78604709862923189bad94014bef562cc35cf61d3a07bba02a7ed     --hash=sha256:679b0076f67ecc0138fd2ede3a8fd196dddc2ad3254069bcb9faf9a79b1cebcd     --hash=sha256:7349ab0fa0c429c82442a27a9673fc802ffdb7c7775fad780226cb234965e53c     --hash=sha256:7ab55401287bfec946ced39700c053796e7cc0e3acbef09993a9ad2adba6ca6e     --hash=sha256:7e50d0a0cc3189f9cb0aeb3a6a6af18c16f59f004b866cd2be1c14b36134a4a0     --hash=sha256:95a7476c59002f2f6c590b9b7b998306fba6a5aa646b1e22ddfeaf8f78c3a29c     --hash=sha256:96ff0b2ad353d8f990b63294c8986f1ec3cb19d749234014f4e7eb0112ceba5a     --hash=sha256:9fad7dcb1aac3c7f0584a5a8133e3a43eeb2fe127f47e3632d43d677c66c102b     --hash=sha256:9ff0f4f29c51e2803569d7a51c2304de5554655a60c5d776e35b4a41413830d0     --hash=sha256:a354325ee03388678242a4d7ebcd08b5c727033fcff3b2f536aea978e15ee9e6     --hash=sha256:a4abb4f9001ad2858e7ac189089c42178fcce737e4169dc61321660f1a96c7d2     --hash=sha256:ab47dbe5cc8210f55aa58e4805fe224dac469cde56b9f731a4c098b91917159a     --hash=sha256:afedb719a9dcfc7eaf2287b839d8198e06dcd4cb5d276a3df279231138e83d30     --hash=sha256:b3ce300f3644fb06443ee2222c2201dd3a89ea6040541412b8fa189341847218     --hash=sha256:b97fe8060236edf3662adfc2c633f56a08ae30560c56310562cb4f95500022d5     --hash=sha256:bfe25acf8b437eb2a8b2d49d443800a5f18508cd811fea3181723922a8a82b07     --hash=sha256:cd25bcecc4974d09257ffcd1f098ee778f7834c3ad767fe5db785be9a4aa9cb2     --hash=sha256:d209d8969599b27ad20994c8e41936ee0964e6da07478d6c35016bc386b66ad4     --hash=sha256:d5241e0a80d808d70546c697135da2c613f30e28251ff8307eb72ba696945764     --hash=sha256:edd8b5fe47dab091176d21bb6de568acdd906d1887a4584a15a9a96a1dca06ef     --hash=sha256:f870204a840a60da0b12273ef34f7051e98c3b5961b61b0c2c1be6dfd64fbcd3     --hash=sha256:ffa75af20b44f8dba823498024771d5ac50620e6915abac414251bd971b4529f",
  download_only = False,
  enable_implicit_namespace_pkgs = False,
  environment = {  },
  extra_pip_args = [  ],
  isolated = True,
  pip_data_exclude = [  ],
  python_interpreter = "python3",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
  quiet = True,
  repo_prefix = "pip_deps_",
  timeout = 600,
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `pip_deps_requests`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pip_deps_requests instantiated at:
  /home/brendan/pytorch/WORKSPACE:269:13: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pip_deps/requirements.bzl:47:20: in install_deps
Repository rule whl_library defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip_install/pip_repository.bzl:712:30: in <toplevel>

```

#### Definition
```python
load("@@rules_python//python/pip_install:pip_repository.bzl", "whl_library")
whl_library(
  name = "pip_deps_requests",
  repo = "pip_deps",
  requirement = "requests==2.32.2     --hash=sha256:dd951ff5ecf3e3b3aa26b40703ba77495dab41da839ae72ef3c8e5d8e2433289     --hash=sha256:fc06670dd0ed212426dfeb94fc1b983d917c4f9847c863f313c9dfaaffb7c23c",
  download_only = False,
  enable_implicit_namespace_pkgs = False,
  environment = {  },
  extra_pip_args = [  ],
  isolated = True,
  pip_data_exclude = [  ],
  python_interpreter = "python3",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
  quiet = True,
  repo_prefix = "pip_deps_",
  timeout = 600,
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `rules_python`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository rules_python instantiated at:
  /home/brendan/pytorch/WORKSPACE:238:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "rules_python",
  url = "https://github.com/bazelbuild/rules_python/releases/download/0.21.0/rules_python-0.21.0.tar.gz",
  strip_prefix = "rules_python-0.21.0",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `rules_python`

Found partially name matches in BCR: `rules_python_gazelle_plugin`

It has been introduced as a Bazel module:

	bazel_dep(name = "rules_python", version = "1.5.3")
## Migration of `rules_cuda`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository rules_cuda instantiated at:
  /home/brendan/pytorch/WORKSPACE:45:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "rules_cuda",
  urls = [
    "https://github.com/bazel-contrib/rules_cuda/releases/download/v0.2.5/rules_cuda-v0.2.5.tar.gz"
  ],
  sha256 = "fe8d3d8ed52b9b433f89021b03e3c428a82e10ed90c72808cc4988d1f4b9d1b3",
  strip_prefix = "rules_cuda-v0.2.5",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `rules_cuda`

It has been introduced as a Bazel module:

	bazel_dep(name = "rules_cuda", version = "0.2.5")
## Migration of `pip_deps`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pip_deps instantiated at:
  /home/brendan/pytorch/WORKSPACE:261:10: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip.bzl:155:19: in pip_parse
Repository rule pip_repository defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/rules_python/python/pip_install/pip_repository.bzl:612:33: in <toplevel>

```

#### Definition
```python
load("@@rules_python//python/pip_install:pip_repository.bzl", "pip_repository")
pip_repository(
  name = "pip_deps",
  requirements_lock = "//:tools/build/bazel/requirements.txt",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `com_github_google_flatbuffers`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
local_repository(
  name = "com_github_google_flatbuffers",
  path = "third_party/flatbuffers",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension since it is local_repository rule:

## Migration of `bazel_skylib`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository bazel_skylib instantiated at:
  /home/brendan/pytorch/WORKSPACE:17:27: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/build_bazel_apple_support/lib/repositories.bzl:39:11: in apple_support_dependencies
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/build_bazel_apple_support/lib/repositories.bzl:29:18: in _maybe
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "bazel_skylib",
  urls = [
    "https://github.com/bazelbuild/bazel-skylib/releases/download/1.3.0/bazel-skylib-1.3.0.tar.gz",
    "https://mirror.bazel.build/github.com/bazelbuild/bazel-skylib/releases/download/1.3.0/bazel-skylib-1.3.0.tar.gz"
  ],
  sha256 = "74d544d96f4a5bb630d465ca8bbcfe231e3594e5aae57e1edbf17a6eb3ca2506",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `bazel_skylib`

Found partially name matches in BCR: `bazel_skylib_gazelle_plugin`

It has been introduced as a Bazel module:

	bazel_dep(name = "bazel_skylib", version = "1.8.1")
## Migration of `rules_cc`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository rules_cc instantiated at:
  /home/brendan/pytorch/WORKSPACE:17:27: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/build_bazel_apple_support/lib/repositories.bzl:67:11: in apple_support_dependencies
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/build_bazel_apple_support/lib/repositories.bzl:29:18: in _maybe
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "rules_cc",
  url = "https://github.com/bazelbuild/rules_cc/releases/download/0.0.8/rules_cc-0.0.8.tar.gz",
  sha256 = "ae46b722a8b8e9b62170f83bfb040cbf12adb732144e689985a66b26410a7d6f",
  strip_prefix = "rules_cc-0.0.8",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `rules_cc`

It has been introduced as a Bazel module:

	bazel_dep(name = "rules_cc", version = "0.2.0")
## Migration of `pybind11_bazel`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository pybind11_bazel instantiated at:
  /home/brendan/pytorch/WORKSPACE:69:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "pybind11_bazel",
  urls = [
    "https://github.com/pybind/pybind11_bazel/archive/b162c7c88a253e3f6b673df0c621aca27596ce6b.zip"
  ],
  strip_prefix = "pybind11_bazel-b162c7c88a253e3f6b673df0c621aca27596ce6b",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `pybind11_bazel`

It has been introduced as a Bazel module:

	bazel_dep(name = "pybind11_bazel", version = "2.13.6")
## Migration of `cudnn`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository cudnn instantiated at:
  /home/brendan/pytorch/WORKSPACE:288:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "cudnn",
  url = "https://developer.download.nvidia.com/compute/cudnn/redist/cudnn/linux-x86_64/cudnn-linux-x86_64-9.12.0.46_cuda12-archive.tar.xz",
  strip_prefix = "cudnn-linux-x86_64-9.12.0.46_cuda12-archive",
  build_file = "//third_party:cudnn.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced with `use_repo_rule`:

## Migration of `cuda`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "cuda",
  path = "/usr/local/cuda",
  build_file = "//third_party:cuda.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `eigen`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository eigen instantiated at:
  /home/brendan/pytorch/WORKSPACE:128:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "eigen",
  url = "https://gitlab.com/libeigen/eigen/-/archive/3.3.7/eigen-3.3.7.zip",
  build_file = "//third_party:eigen.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `eigen`

It has been introduced as a Bazel module:

	bazel_dep(name = "eigen", version = "4.0.0-20241125.bcr.2")
## Migration of `cutlass`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "cutlass",
  path = "third_party/cutlass",
  build_file = "//third_party:cutlass.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `cutlass`

It has been introduced as a Bazel module:

	bazel_dep(name = "cutlass", version = "3.5.1")
## Migration of `nlohmann`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "nlohmann",
  path = "third_party/nlohmann",
  build_file = "//third_party:nlohmann.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `kineto`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "kineto",
  path = "third_party/kineto",
  build_file = "//third_party:kineto.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `kineto`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "kineto",
  path = "third_party/kineto",
  build_file = "//third_party:kineto.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `kineto`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "kineto",
  path = "third_party/kineto",
  build_file = "//third_party:kineto.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `cpp-httplib`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "cpp-httplib",
  path = "third_party/cpp-httplib",
  build_file = "//third_party:cpp-httplib.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `cpp-httplib`

It has been introduced as a Bazel module:

	bazel_dep(name = "cpp-httplib", version = "0.22.0")
## Migration of `fmt`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "fmt",
  path = "third_party/fmt",
  build_file = "//third_party:fmt.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `fmt`

It has been introduced as a Bazel module:

	bazel_dep(name = "fmt", version = "11.2.0")
## Migration of `tensorpipe`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "tensorpipe",
  path = "third_party/tensorpipe",
  build_file = "//third_party:tensorpipe.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `fbgemm`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "fbgemm",
  path = "third_party/fbgemm",
  build_file = "//third_party:fbgemm/BUILD.bazel",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `onnx`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "onnx",
  path = "third_party/onnx",
  build_file = "//third_party:onnx.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `onnx`

It has been introduced as a Bazel module:

	bazel_dep(name = "onnx", version = "1.17.0")
## Migration of `com_google_protobuf`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
local_repository(
  name = "com_google_protobuf",
  path = "third_party/protobuf",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension since it is local_repository rule:

## Migration of `cudnn_frontend`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "cudnn_frontend",
  path = "third_party/cudnn_frontend/",
  build_file = "//third_party:cudnn_frontend.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `ideep`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "ideep",
  path = "third_party/ideep",
  build_file = "//third_party:ideep.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `mkl_dnn`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "mkl_dnn",
  path = "third_party/ideep/mkl-dnn",
  build_file = "//third_party:mkl-dnn.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `mkl_dnn`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "mkl_dnn",
  path = "third_party/ideep/mkl-dnn",
  build_file = "//third_party:mkl-dnn.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `sleef`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "sleef",
  path = "third_party/sleef",
  build_file = "//third_party:sleef.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `local_config_python`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository local_config_python instantiated at:
  /home/brendan/pytorch/WORKSPACE:273:17: in <toplevel>
Repository rule python_configure defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pybind11_bazel/python_configure.bzl:412:35: in <toplevel>

```

#### Definition
```python
load("@@pybind11_bazel//:python_configure.bzl", "python_configure")
python_configure(
  name = "local_config_python",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `local_config_python`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository local_config_python instantiated at:
  /home/brendan/pytorch/WORKSPACE:273:17: in <toplevel>
Repository rule python_configure defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pybind11_bazel/python_configure.bzl:412:35: in <toplevel>

```

#### Definition
```python
load("@@pybind11_bazel//:python_configure.bzl", "python_configure")
python_configure(
  name = "local_config_python",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `local_config_python`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository local_config_python instantiated at:
  /home/brendan/pytorch/WORKSPACE:273:17: in <toplevel>
Repository rule python_configure defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pybind11_bazel/python_configure.bzl:412:35: in <toplevel>

```

#### Definition
```python
load("@@pybind11_bazel//:python_configure.bzl", "python_configure")
python_configure(
  name = "local_config_python",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `local_config_python`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository local_config_python instantiated at:
  /home/brendan/pytorch/WORKSPACE:273:17: in <toplevel>
Repository rule python_configure defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/pybind11_bazel/python_configure.bzl:412:35: in <toplevel>

```

#### Definition
```python
load("@@pybind11_bazel//:python_configure.bzl", "python_configure")
python_configure(
  name = "local_config_python",
  python_interpreter_target = "@@python3_10_x86_64-unknown-linux-gnu//:bin/python3",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced using a module extension:

## Migration of `asmjit`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "asmjit",
  path = "third_party/fbgemm/external/asmjit",
  build_file = "//third_party:fbgemm/external/asmjit.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `rules_proto`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository rules_proto instantiated at:
  /home/brendan/pytorch/WORKSPACE:280:14: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/com_google_protobuf/protobuf_deps.bzl:52:21: in protobuf_deps
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "rules_proto",
  urls = [
    "https://github.com/bazelbuild/rules_proto/archive/97d8af4dc474595af3900dd85cb3a29ad28cc313.tar.gz"
  ],
  sha256 = "602e7161d9195e50246177e7c55b2f39950a9cf7366f74ed5f22fd45750cd208",
  strip_prefix = "rules_proto-97d8af4dc474595af3900dd85cb3a29ad28cc313",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `rules_proto`

It has been introduced as a Bazel module:

	bazel_dep(name = "rules_proto", version = "7.1.0")
## Migration of `mkl`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository mkl instantiated at:
  /home/brendan/pytorch/WORKSPACE:219:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "mkl",
  urls = [
    "https://anaconda.org/anaconda/mkl/2020.0/download/linux-64/mkl-2020.0-166.tar.bz2"
  ],
  sha256 = "59154b30dd74561e90d547f9a3af26c75b6f4546210888f09c9d4db8f4bf9d4c",
  strip_prefix = "lib",
  build_file = "//third_party:mkl.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced with `use_repo_rule`:

## Migration of `com_github_glog`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository com_github_glog instantiated at:
  /home/brendan/pytorch/WORKSPACE:81:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "com_github_glog",
  urls = [
    "https://github.com/google/glog/archive/v0.4.0.tar.gz"
  ],
  strip_prefix = "glog-0.4.0",
  build_file_content = "\nlicenses(['notice'])\n\nload(':bazel/glog.bzl', 'glog_library')\n# TODO: figure out why enabling gflags leads to SIGSEV on the logging init\nglog_library(with_gflags=0)\n    ",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found partially name matches in BCR: `glog`

It has been introduced as a Bazel module:

	bazel_dep(name = "glog", version = "0.7.1", repo_name = "com_github_glog")
## Migration of `com_github_gflags_gflags`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository com_github_gflags_gflags instantiated at:
  /home/brendan/pytorch/WORKSPACE:96:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "com_github_gflags_gflags",
  urls = [
    "https://github.com/gflags/gflags/archive/v2.2.2.tar.gz"
  ],
  strip_prefix = "gflags-2.2.2",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found partially name matches in BCR: `gflags`

It has been introduced as a Bazel module:

	bazel_dep(name = "gflags", version = "2.2.2", repo_name = "com_github_gflags_gflags")
Repository definition for `cpuinfo` is not found in ./resolved_deps.py file, please add `--force/-f` flag to force update it.
## Migration of `moodycamel`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "moodycamel",
  path = "third_party/concurrentqueue",
  build_file = "//third_party:moodycamel.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `moodycamel`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "moodycamel",
  path = "third_party/concurrentqueue",
  build_file = "//third_party:moodycamel.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
Repository definition for `cpuinfo` is not found in ./resolved_deps.py file, please add `--force/-f` flag to force update it.
## Migration of `pybind11`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python

```

#### Definition
```python
new_local_repository(
  name = "pybind11",
  path = "third_party/pybind11",
  build_file = "@@pybind11_bazel//:pybind11.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	Please manually add this dependency.
## Migration of `zlib`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository zlib instantiated at:
  /home/brendan/pytorch/WORKSPACE:280:14: in <toplevel>
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/com_google_protobuf/protobuf_deps.bzl:19:21: in protobuf_deps
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "zlib",
  urls = [
    "https://github.com/madler/zlib/archive/v1.2.11.tar.gz"
  ],
  sha256 = "629380c90a77b964d896ed37163f5c3a34f6e6d897311f1df2a7016355c45eff",
  strip_prefix = "zlib-1.2.11",
  build_file = "@@com_google_protobuf//:third_party/zlib.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
Found perfect name match in BCR: `zlib`

It has been introduced as a Bazel module:

	bazel_dep(name = "zlib", version = "1.3.1.bcr.6")
## Migration of `mkl_headers`:

<details>
<summary>Click here to see where and how the repo was declared in the WORKSPACE file</summary>

#### Location
```python
Repository mkl_headers instantiated at:
  /home/brendan/pytorch/WORKSPACE:229:13: in <toplevel>
Repository rule http_archive defined at:
  /home/brendan/.cache/bazel/_bazel_brendan/89f6f0211654284fabe9802449168206/external/bazel_tools/tools/build_defs/repo/http.bzl:387:31: in <toplevel>

```

#### Definition
```python
load("@@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")
http_archive(
  name = "mkl_headers",
  urls = [
    "https://anaconda.org/anaconda/mkl-include/2020.0/download/linux-64/mkl-include-2020.0-166.tar.bz2"
  ],
  sha256 = "2af3494a4bebe5ddccfdc43bacc80fcd78d14c1954b81d2c8e3d73b55527af90",
  build_file = "//third_party:mkl_headers.BUILD",
)
```
**Tip**: URLs usually show which version was used.
</details>

___
	It is not found in BCR. 

	It has been introduced with `use_repo_rule`:

